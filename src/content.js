...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "SHOW_STICKER" && message.imageUrl) {
    ...
    console.log("Message type or imageUrl missing:", {
      type: message.type,
...
const showUploadOverlay = () => {
  ...
  if (modalContent) {
    modalContent.style.backgroundImage = `url('${chrome.runtime.getURL(
  ...
  overlay
    .querySelector(".switch-upload-button")
    .addEventListener("click", () => showGenerateOverlay());
  ...
  confirmButton.addEventListener("click", async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://xrvicqszlafncvfmqydp.supabase.co/functions/v1/smooth-function",
        {
          method: "POST",
          body: formData,
        }
...
const showGenerateOverlay = () => {
  ...
  overlay
    .querySelector(".switch-upload-button")
    .addEventListener("click", () => showUploadOverlay());
  overlay.addEventListener("click", (e) => {
    if (e.target === overlay) overlay.remove();
  });
  ...
  confirmButton.addEventListener("click", async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://xrvicqszlafncvfmqydp.supabase.co/functions/v1/generate-sticker-via-prompt",
        {
          method: "POST",
          headers: headers,
